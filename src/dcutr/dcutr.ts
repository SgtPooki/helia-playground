...
import { HolePunch } from './message.js'
import {
  multicodec
} from './index.js'
...
// import { Topology } from '@libp2p/interface/topology'
...
debug.enable('libp2p:dcutr*,libp2p:dcutr:trace,libp2p:dcutr:A:trace,libp2p:dcutr:B:trace')

const log = debug('libp2p:dcutr');

(log as any).trace = log.extend('trace');
(log as any).error = log.extend('error');
const logA = debug('libp2p:dcutr:A');
(logA as any).trace = logA.extend('trace');
(logA as any).error = logA.extend('error');

const logB = debug('libp2p:dcutr:B');
(logB as any).trace = logB.extend('trace');
(logB as any).error = logB.extend('error');
...
export class DefaultDCUtRService implements Startable {
  ...
  async start (): Promise<void> {
    ...
    this.topologyId = await this.registrar.register(multicodec, dcutrTopology)
  ...
  async initiateDCUtRUpgrade (relayedConnection: Connection): Promise<void> {
    logB.trace('initiateDCUtRUpgrade connection ID =', relayedConnection.id)
    /**
    ...
    for (let i = 0; i < this.retries; i++) {
      ...
      try {
        ...
        logB.trace('opening a stream to A %s', relayedConnection.remotePeer)
        ...
        logB.trace('sending CONNECT to A %s', relayedConnection.remotePeer)
        ...
        logB.trace('waiting for CONNECT from A %s', relayedConnection.remotePeer)
        // 4. Upon receiving the Connect, B sends a Sync message
        const connect = await pb.read(options)
        logB.trace('received CONNECT from A %s', relayedConnection.remotePeer)
  ...
  getDialableMultiaddrs (addrs: Array<Uint8Array | string | null | undefined>): Multiaddr[] {
    ...
    for (const addr of addrs) {
      ...
      try {
        ...
        log.error('error parsing multiaddr', err)
      }
    }
  ...
  isPublicAndDialable (ma: Multiaddr): boolean {
    ...
    if (Circuit.matches(ma)) {
      log.trace('ignoring circuit relay address', ma.toString())
      return false
    }

    // dns addresses are probably public?
    if (DNS.matches(ma)) {
      log.trace('ignoring dns address', ma.toString())
      return true
    }

    // ensure we have only IPv4/IPv6 addresses
    if (!IP.matches(ma)) {
      log.trace('ignoring non-ip address', ma.toString())
      return false
    }
    ...
    if (transport == null) {
      log.trace('ignoring address with no transport', ma.toString())
      return false
    }
    ...
    if (isPrivate(options.host) === true) {
      log.trace('ignoring private address', ma.toString())
      return false
    }
