...
const debug = logger('libp2p:webtransport:debug')
debug('WebTransport is', typeof WebTransport === 'undefined' ? 'not' : '', 'available')
...
const pendingWebTransportSessions: Map<string, InstanceType<typeof WebTransport>> = new Map()
const uniqueWebTransportUrls: Set<string> = new Set()

class WebTransportDebugger {
...
export interface WebTransportInit {
  ...
  maxWebTransportSessions?: number
}
...
class WebTransportTransport implements Transport {
  ...
  constructor (components: WebTransportComponents, init: WebTransportInit = {}) {
    ...
    this.config = {
      ...
      maxWebTransportSessions: init.maxWebTransportSessions ?? 60
    }
  }
  ...
  async dial (ma: Multiaddr, options: DialOptions): Promise<Connection> {
    ...
    if (pendingWebTransportSessions.size >= this.config.maxWebTransportSessions) {
      debug('Preventing WebTransport dial due to too many pending sessions')
      throw new Error('Too many pending webtransport sessions')
    }
    // if (pendingWebTransportSessions.has(url)) {
    //   debug('Preventing WebTransport dial due to existing pending session')
    //   throw new Error('WebTransport session already pending')
    // }

    let wt: null | InstanceType<typeof WebTransport> = null
    try {
      ...
      debug('Failed to create new WebTransport instance', e)
      throw e
    }
    if (wt == null) {
      throw new Error('Failed to create new WebTransport instance')
    }
    uniqueWebTransportUrls.add(url)
    debug('uniqueWebTransportUrls.size: ', uniqueWebTransportUrls.size)
    pendingWebTransportSessions.set(url, wt)
    debug('pendingWebTransportSessions.size: ', pendingWebTransportSessions.size)
    // wt.closed.catch((error: Error) => {
    //   debug('WebTransport transport closed due to:', error)
    //   pendingWebTransportSessions.delete(url)
    // })
    ...
    if (!await this.authenticateWebTransport(wt, localPeer, remotePeer, certhashes)) {
      debug('Failed to authenticate webtransport')
      pendingWebTransportSessions.delete(url)
      wt.close()
  ...
  webtransportMuxer (wt: InstanceType<typeof WebTransport>): StreamMuxerFactory {
    ...
    return {
      ...
      createStreamMuxer: (init?: StreamMuxerInit): StreamMuxer => {
        ...
        debug(`activeStreams.length: `, activeStreams.length);
